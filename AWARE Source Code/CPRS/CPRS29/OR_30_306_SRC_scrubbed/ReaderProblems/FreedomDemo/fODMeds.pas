...
uses
  ...
  StdCtrls, ComCtrls, ExtCtrls, ORCtrls, Grids, Buttons, ORDtTm,
  Menus, VA508AccessibilityManager, VAUtils;

const
  UM_DELAYCLICK = 11037;  // temporary for listview click event

type
  TfrmODMeds = class(TForm)
    txtMed: TEdit;
    pnlMeds: TPanel;
    btnSelect: TButton;
    pnlFields: TPanel;
...
uses { rCore, uCore, rODMeds, rODBase, rOrders, fRptBox, fODMedOIFA,
  uOrders, fOtherSchedule, fFrame, } ORFn, StrUtils, VA508AccessibilityRouter;
    ...
    if (FInptDlg) and (OrderAction in [ORDER_COPY, ORDER_EDIT])  then
      begin
        ...
        if (FAdminTimeText <> 'Not defined for Clinic Locations') and (self.tabDose.TabIndex = TI_COMPLEX) then
            self.lblAdminSch.Caption := '';
        if (FAdminTimeText <> '') and (self.tabDose.TabIndex = TI_DOSE) then self.lblAdminSch.Caption := 'Admin. Time: ' + FAdminTimeText;
      end;
    ...
    begin
       for i := 1 to self.grdDoses.RowCount do
         begin
           if i = 1 then
              begin
                if LeftStr(ValFor(COL_DOSAGE, i),1) = '.' then
                  begin
                     SetError('Dosage must have a leading numeric value');
                     Exit;
                  end;
...
begin
  ...
  if First = last then
  begin
    BrokerResult.Add('2292^AMOXICILLIN/CLARITHROMYCIN/LANSOPRAZOLE MISCELLANEOUS      NF');
  end
  else
  begin
    BrokerResult.Add('1431^5% LCD CREAM,TOP');
    BrokerResult.Add('2334^ABACAVIR SOLN,ORAL');
    BrokerResult.Add('1358^ACCUPRIL     <QUINAPRIL TAB >');
    BrokerResult.Add('1286^ACETAMMMMINOPHEN TAB');
    BrokerResult.Add('1284^ACETAMMMMINOPHEN/CODEINE ELIXIR');
    BrokerResult.Add('1285^ACETAMMMMINOPHEN/CODEINE TAB');
    BrokerResult.Add('1406^ACYCLOVIR CAP,ORAL');
    BrokerResult.Add('1288^ALBUTEROL INHALANT');
    BrokerResult.Add('1289^ALLOPURINOL TAB');
    BrokerResult.Add('2313^ALPRAZOLAM TAB      NF');
    BrokerResult.Add('1336^ALUPENT     <METAPROTERNOL AEROSOL >');
    BrokerResult.Add('1291^AMINOPHYLLINE TAB');
    BrokerResult.Add('1418^AMOX & K CLAVULANATE     <AMOXICILLIN/CLAVULANATE TAB >');
    BrokerResult.Add('1292^AMOXAPINE TAB');
    BrokerResult.Add('1418^AMOXICILLIN & K CLAVULANATE     <AMOXICILLIN/CLAVULANATE TAB >');
    BrokerResult.Add('1293^AMOXICILLIN AND CLAVULANIC ACID TAB');
    BrokerResult.Add('2292^AMOXICILLIN/CLARITHROMYCIN/LANSOPRAZOLE MISCELLANEOUS      NF');
    BrokerResult.Add('1418^AMOXICILLIN/CLAVULANATE TAB');
    BrokerResult.Add('1294^AMPICILLIN CAP,ORAL');
    BrokerResult.Add('1295^AMPICILLIN INJ');
    BrokerResult.Add('1292^ASCENDIN     <AMOXAPINE TAB >');
    BrokerResult.Add('1411^ASPIRIN     <ASPIRIN TAB >');
    BrokerResult.Add('1410^ASPIRIN     <ASPIRIN TAB,EC >     NF');
    BrokerResult.Add('1412^ASPIRIN SUPP,RTL');
    BrokerResult.Add('1411^ASPIRIN TAB');
    BrokerResult.Add('1410^ASPIRIN TAB,EC      NF');
    BrokerResult.Add('1332^ATIVAN     <ATIVAN2 TAB >');
    BrokerResult.Add('1407^ATIVAN     <LORAZEPAM INJ >');
    BrokerResult.Add('1332^ATIVAN2 TAB');
    BrokerResult.Add('1293^AUGMENTIN     <AMOXICILLIN AND CLAVULANIC ACID TAB >');
    BrokerResult.Add('1418^AUGMENTIN     <AMOXICILLIN/CLAVULANATE TAB >');
    BrokerResult.Add('1293^AUGMENTIN 250     <AMOXICILLIN AND CLAVULANIC ACID TAB >');
    BrokerResult.Add('1293^AUGMENTIN 500     <AMOXICILLIN AND CLAVULANIC ACID TAB >');
    BrokerResult.Add('1297^AZATHIOPRINE TAB');
    BrokerResult.Add('2241^B1     <THIAMINE INJ,SOLN >');
    BrokerResult.Add('1366^B1     <THIAMINE TAB >');
    BrokerResult.Add('2264^BACTRIM     <SULFAMETHOXAZOLE/TRIMETHOPRIM TAB >');
    BrokerResult.Add('2331^BECLOMETHASONE DIPROPIONATE AEROSOL,ORAL      NF');
    BrokerResult.Add('1366^BIG TEST     <THIAMINE TAB >');
    BrokerResult.Add('1299^BISACODYL TAB,EC');
  end;
  if Append then
    FastAddStrings(BrokerResult, Dest)
  else
  begin
    for i := Pred(BrokerResult.Count) downto 0 do Dest.Insert(0, BrokerResult[i]);
  end;
end;
...
//        CheckFormularyOI(MedIEN, temp, FInptDlg);
      ...
      SetOnMedSelect;
      ShowMedFields;
cboDosage.Items.Add('ACETAMMMMINOPHEN 325MG/30MG COD TAB #3^^NF^&&&&1 TABLET&92&&^1 TABLET & ACETAMMMMINOPHEN 325MG/30MG COD TAB #3^^5^TAB');
cboDosage.Items.Add('ACETAMMMMINOPHEN 325MG/30MG COD TAB #3^^NF^&&&&2 TABLETS&92&&^&2 TABLETS & ACETAMMMMINOPHEN 325MG/30MG COD TAB #3^^5^TAB');
...
//  Responses.Clear('QTY');
  ...
  cboSchedule.ItemIndex := -1;
  cboSchedule.Text := '';  // leave items intact
  if FAdminTimeText <> 'Not defined for Clinic Locations' then lblAdminSch.Caption := '';
  txtSupply.Text := '';
  txtSupply.Tag := 0;
  txtQuantity.Text := '';
  txtQuantity.Tag := 0;
...
//    FGuideline.Text := TextOf('Guideline');
    ...
    begin
      ...
      lblAdminSch.Height := MainFontHeight + 2;
      lblAdminTime.Left := lblAdminSch.Left;
      lblAdminTime.top := lblAdminSch.Top + lblAdminSch.Height -1;
      if self.tabDose.TabIndex = TI_Dose then lblAdminSch.Caption := ''
      else
        begin
          if FAdminTimeText = 'Not defined for Clinic Locations' then self.lblAdminSch.Caption := 'Admin. Time: ' + FAdminTimeText
...
begin
  ...
  TI_COMPLEX: begin
                ...
                tmpAdmin := Piece(self.lblAdminSch.Caption, ':', 2);
...
//      x := ValueOf(FLD_SCHEDULE);    Responses.Update('SCHEDULE', 1, x,  x);
      if FInptDlg then
        begin
          x := Piece(self.lblAdminSch.Caption,':',2);
          x := Copy(x,2,Length(x));
...
var
  CompSch, CompDose, LastSch,ShowText, Duration, ASchedule, TempSch, schType, Admin: string;
...
//  if (FAdminTimeText = '') and (self.EvtID > 0) then FAdminTimeText := 'To Be Determined';
    ...
    begin
     ...
     if self.chkPRN.Checked = True then
       begin
         ...
         if cboSchedule.ItemIndex > -1 then
           begin
             SchType := Piece(self.cboSchedule.Items.Strings[cboSchedule.itemIndex], U, 3);
             if (SchType = 'P') or (SchType = 'OC') or (SchType = 'O')  then
               ...
               begin
                 if FAdminTimeText <> '' then self.lblAdminSch.Caption := 'Admin. Time: ' + FAdminTimeText
                 else
                   begin
                      if Piece(self.cboSchedule.Items.Strings[cboSchedule.itemIndex], U, 4) <> '' then
                          self.lblAdminSch.Caption := 'Admin. Time: ' + Piece(self.cboSchedule.Items.Strings[cboSchedule.itemIndex], U, 4)
                      else self.lblAdminSch.Caption := 'Admin. Time: Not Defined';
                   end;
  ...
  if (Length(ASchedule)>0) and (AdminTime > -1) then
     begin
      ...
      Admin := '';
      if (self.lblAdminSch.visible = True) and (self.lblAdminSch.Caption <> '') and (self.tabDose.TabIndex = TI_DOSE) then
        begin
          Admin := Copy(self.lblAdminSch.Caption,  14, (Length(self.lblAdminSch.Caption)-1));
...
//      LoadAdminInfo(ASchedule, txtMed.Tag, ShowText, AdminTime, Duration, Admin);
  ...
  begin
    ...
    Interval := Trunc(FMDateTimeToDateTime(AdminTime) - FMDateTimeToDateTime(NOW));
    case Interval of
  ...
  if (lblAdminSch.Caption <> '') and (lblAdminSch.Visible = True) and (JAWSON = true) then lblAdminSch.TabStop := true
...
initialization
  BrokerResult := TStringList.Create;

finalization
  FreeAndNil(BrokerResult);
