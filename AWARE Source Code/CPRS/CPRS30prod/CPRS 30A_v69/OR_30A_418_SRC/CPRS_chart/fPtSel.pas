...
var
  ...
  PtStrs: TStringList;
  tTabsheet : integer;  //AWARE  change
...
uses rCore, uCore, fDupPts, fPtSens, fPtSelDemog, fPtSelOptns, fPatientFlagMulti,
     uOrPtf, fAlertForward, rMisc, fFrame, fRptBox, VA508AccessibilityRouter,
         VAUtils,
       uEventHooks,fNotes,fDrawers,fPage,fEncnt;   //AWARE change
     //VAUtils;  PREVIOUS

var
  FDragging: Boolean = False;
  //AWARE change below
  StartNode: TTreeNode = nil ;
  //end AWARE change
const
  AliasString = ' -- ALIAS';
    //AWARE changes below
  Param2 = '' ;
  Param3 = '' ;
//AWARE change start below

procedure SelectPatient(ShowNotif: Boolean; FontSize: Integer; var UserCancelled: boolean);
{ displays patient selection dialog (with optional notifications), updates Patient object }
var
  frmPtSel: TfrmPtSel;
  //AWARE changes below declarations
  Data1,Data2 : string;
  Found : Boolean;
  ORTreeNode,ORTreeNode1:TORTreeNode ;
  TIUTemplate: String;
  strtmp1: String;
  //end AWARE changes
begin
  ...
  //end AWARE change
  //AWARE changes start below

  if frmPtSelDemog = nil then
  begin
          //frmPtSelDemog.CheckPrevID()  then
          tTabSheet := 0 ;  //tabClinic on fEncnt
          frmPtSel := TfrmPtSel.Create(Application);
  end
  else
  begin
                ...
                Data2 := '';
                tTabSheet := 0 ;  //tabClinic on fEncnt
                ProcessPatientChartCloseoutEventHook(Param2, Param3,Data1, Data2);
                if (Data1='') and (Data2='') then
                begin
                  //ShowMessage('fPtsel');
                  frmFrame.mnuChartTabClick(frmFrame.mnuChartCover) ;
                  Application.ProcessMessages;
                  //frmFrame.mnuChartTabClick(frmFrame.mnuChartCover) ;
                  //NextTab := frmFrame. TabToPageID(tabPage.TabIndex);
                  //frmFrame.DetermineNextTab    ;
                  //NextTab := frmFrame.TabToPageID(frmFrame.tabPage.TabIndex)  ;
                  FrmFrame.tabPage.TabIndex := FrmFrame.PageIDToTab(CT_COVER);
                  Application.ProcessMessages;
                  { NextTab := FrmFrame.TabToPageID(FrmFrame.tabPage.TabIndex) ;
                  ...
                  frmFrame.SwitchToPage(frmCover); }
                  //fPage.frmPage.PageID := CT_COVER;
                ...
                begin
                  Found := False;
                  fNotes.frmDrawers1.tvTemplates.FullCollapse ;
                  Application.ProcessMessages;
                  fNotes.frmDrawers1.OpenDrawer(odNone) ;
                  Application.ProcessMessages;
                  frmFrame.mnuChartTabClick(frmFrame.mnuChartCover) ;
                  //ShowMessage('herre-2');
                  Application.ProcessMessages;
                  //strtmp1 := Data2[1]='O','Ord',2) ;
                  if Data2[1]='O' then    // ;  //ORDERS NEED COMPLETING/SIGNING
                  begin
                        frmFrame.mnuChartTabClick(frmFrame.mnuChartOrders);  //   mnuChartNotes);
                        Application.ProcessMessages;
                  end
                  else
                  begin
                  tTabSheet := 1 ;  //tabNewVisit on fEncnt
                  if Encounter <> nil then
                  begin
                    //Encounter.Location := 0;
                    //Encounter.LocationName := '';
                    //Encounter.LocationText := '';
                    //ShowMessage(InttoStr(Encounter.DateTime));
                    Encounter.DateTime := FMNow;   //0
                    //Encounter.VisitCategory := #0;
                  end;
                  frmFrame.mnuChartTabClick(frmFrame.mnuChartNotes); //;frmNotes.mnuChartNotes// mnuViewChart mnuChartNotes) ;
                  try
                  while (not Found) do
                    begin

                      Application.ProcessMessages;
                      //fNotes.frmNotes.cmdNewNoteClick(frmNotes);    Worked


                      fNotes.frmDrawers1.sbTemplatesClick(fNotes.frmNotes);//   pnlTemplatesButtonEnter(fNotes.frmNotes)   ;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.OpenDrawer(odTemplates) ;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplates.Show ; //ShowRoot := True;  //ShowRoot;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplates.FullExpand  ;

                      Application.ProcessMessages;
                      fNotes.frmDrawers1.DisplayDrawers(TRUE, [odTemplates], [odTemplates]); //FALSE);
                      Application.ProcessMessages;
                      //fNotes.frmDrawers.tvTemplates.VertScrollPos := 0;  //fNotes.frmDrawers. tvTemplates
                      Application.ProcessMessages;
                      TIUTemplate := Piece(Data1,'^',2)  ;
                      //ORTreeNode:= fNotes.frmDrawers.tvTemplates.FindPieceNode('VEFA PSA',#0, StartNode); //;
                      ORTreeNode:= fNotes.frmDrawers1.tvTemplates.FindPieceNode(TIUTemplate,#0, StartNode); //;
                      //ShowMessage(ORTreeNode.Text);
                      Application.ProcessMessages;
                      //fNotes.frmDrawers.tvTemplates.TopItem := ORTreeNode   ;
                      Application.ProcessMessages;
                      ORTreeNode.Selected :=True;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplates.Selected := ORTreeNode;  //   CanFocus := True;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplates.RowSelect := True ; //
                      Application.ProcessMessages;
                      ORTreeNode.Focused := True;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplates.SetFocus;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplates.Selected.Focused ; //  VertScrollPos :=0 ; //Select(ORTreeNode); //TshiftState      //VertScrollPos :=0 ;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplatesDblClick(fNotes.frmNotes) ;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplates.TopItem := ORTreeNode ;
                      Application.ProcessMessages;
                      //ORTreeNode1:= fNotes.frmDrawers.tvTemplates.FindPieceNode('Shared Templates',#0, StartNode);
                      //ORTreeNode1.Selected :=True;
                      fNotes.frmDrawers1.tvTemplates.Show ; //ShowRoot := True;  //ShowRoot;
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplatesClick(fNotes.frmNotes);
                      Application.ProcessMessages;
                      fNotes.frmDrawers1.tvTemplatesDblClick(fNotes.frmDrawers1.tvTemplates.Selected) ; //(fNotes.frmDrawers.tvTemplates.) ;
                      Application.ProcessMessages;
