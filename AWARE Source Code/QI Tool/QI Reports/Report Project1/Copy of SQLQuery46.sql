DROP TABLE ClinicType3$ SELECT DISTINCT   Consult$.ConsultSID,Consult$.STA3N,Consult$.PatientLocationSID, Consult$.RequestDateTime, Consult$.FromLocationSID, 
                      ...
                      TestStopCOdeDim$.StopCodeName,ConsultActivity$.Activity,TestAppointment$.VisitSID,ConsultActivity$.FromRequestServiceSID
INTO ClinicType3$
FROM          (Consult$ 
                        INNER JOIN
                      ...
                      TestOutpatientEncounter$ on Consult$.PatientSID = TestOutpatientEncounter$.PatientSID
                        INNER JOIN
                      TestAppointment$  ON TestOutpatientEncounter$.PatientSID =  TestAppointment$.PatientSID
                        JOIN
...
WHERE  (Consult$.ToRequestServiceName LIKE '%MH%' OR Consult$.ToRequestServiceName LIKE '%MENTAL%' OR Consult$.ToRequestServiceName LIKE '%PSY%') AND(TestInstitutionDIM$.InstitutionIEN = '580') AND (TestOutpatientEncounter$.VisitVistaDate<(Consult$.FileEntryVistaDate +130))
                      AND (TestOutpatientEncounter$.VisitVistaDate>=(Consult$.FileEntryVistaDate))  AND (TestOutpatientEncounter$.VisitVistaDate<(Consult$.FileEntryVistaDate +130)) AND (((TestOutpatientEncounter$.PrimaryStopCode >499) AND (TestOutpatientEncounter$.PrimaryStopCode <600))OR ((TestOutpatientEncounter$.PrimaryStopCode >210) AND (TestOutpatientEncounter$.PrimaryStopCode <231)))
