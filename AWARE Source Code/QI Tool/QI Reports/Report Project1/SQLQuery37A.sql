...
WHERE ((Cast (Substring(STR(ClinicType22$.VisitVistaDate,11,3),1,3)as INT)+1700) = YEAR(ClinicType22$.AppointmentDateTime))AND (Cast (Substring(STR(ClinicType22$.VisitVistaDate,11,3),4,2)as INT) = MONTH(ClinicType22$.AppointmentDateTime)) AND
      ...
      (Substring(STR(ClinicType22$.VisitVistaDate,11,3),9,2) = Substring(Cast(Convert(time,ClinicType22$.AppointmentDateTime)as NVARCHAR),1,2))
      AND (Substring(STR(ClinicType22$.VisitVistaDate,11,3),11,1) = Substring(Cast(Convert(time,ClinicType22$.AppointmentDateTime)as NVARCHAR),4,1)) 