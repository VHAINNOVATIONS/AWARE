...
IF OBJECT_ID (N'dbo.ufnConfirmeEstablishedPatient', N'FN') IS NOT NULL
    DROP FUNCTION ufnConfirmEstablishedPatient;
GO
CREATE FUNCTION dbo.ufnConfirmEstablishedPatient(@VisitVistaDate float,@AppointmentDateTime datetime)
RETURNS int
AS 
-- Returns comparison of date ( year, month, day and hours and minutes) for 2 dates passed as 'true' or 'false'
-- if any outpatient encounter  found in last 90 days return True (1) else false (0)
BEGIN
    DECLARE @ret int;
    DECLARE @monthday1 int;
    DECLARE @monthday2 int;
...
/* SQL QUERY XX to get "NEW" Patient Type List of patients  */
...
BEGIN
    ...
    CASE 
        WHEN 
            (@ConsultInitiationDate !=NULL) AND (@AppointmentDateTime>@ConsultInitiationDate) AND (@AppointmentDateTime<DATEADD(month,3,@ConsultInitiationDate)
        THEN @AppointmentDateTime
        WHEN 
            (@ConsultInitiationDate =NULL) AND (@AppointmentDateTime>@RequestDateTime) AND (@AppointmentDateTime<DATEADD(month,3,@RequestDateTime)
        THEN @AppointmentDateTime
        ELSE NULL    
    END
    return @ret 
...
WHERE  (TestAppointment$.PatientSID = ClinicType21A$.PatientSID) ) AND
       AND (dbo.ufnNextScheduledAppt(ClinicType21A$.ConsultInitiationDate,TestAppointment$.AppointmentDateTime,ClinicType21A$.RequestDateTime)!=NULL)
